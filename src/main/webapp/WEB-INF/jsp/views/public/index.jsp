...
<div id="carouselExampleCaptions" class="carousel slide">
    <div class="carousel-indicators">
        ...
        <c:forEach var="producto" items="${productosDestacados}" varStatus="loop">
        ...
        <c:choose>
            <c:when test="${not empty productosDestacados}">
                <c:forEach var="producto" items="${productosDestacados}" varStatus="loop">
                    <div class="carousel-item ${loop.first ? 'active' : ''}">
                        ...
                        <c:set var="catalogoPath" value=""/>
                        <c:choose>
                            <c:when test="${producto.tipoProducto == 'Guitarra'}">
                                <c:set var="catalogoPath" value="guitarras"/>
                            </c:when>
                            <c:when test="${producto.tipoProducto == 'Bajo'}">
                                <c:set var="catalogoPath" value="bajos"/>
                            </c:when>
                        </c:choose>


                        <a href="${pageContext.request.contextPath}/catalogos/${catalogoPath}/${fn:toLowerCase(producto.marca)}">


                            <img class="carrusel"
                                 src="${pageContext.request.contextPath}/assets/img/catalogos/${producto.imagenUrl}"
                                 alt="${producto.nombre}">
                        </a>
                        <div class="carousel-caption d-md-block">
                            <h5 class="titulo-carrusel">${producto.nombre}</h5>
                            <p class="subtitulo-carrusel">${producto.descripcion}</p>
                        </div>
                    </div>
                </c:forEach>
            </c:when>
            <c:otherwise>
                ...
                <div class="carousel-item active">
                    ...
                    <img class="carrusel" src="${pageContext.request.contextPath}/assets/img/placeholder.jpg" alt="No hay destacados">
                    <div class="carousel-caption d-md-block">
